...
import {
  clearRoutes,
  setActiveAirport,
} from '../../../redux/features/airports/airportsSlice';
...
export default function AirportCard({ item }: IProps): ReactElement {
  ...
  const onCardHeaderClick = () => {
    ...
    dispatch(
      setActiveAirport(airports.activeAirport === item.id ? null : item.id),
    );
  };
  ...
  return (
    <Card className={classes.card}>
      <Box className={classes.card_header} onClick={onCardHeaderClick}>
        <CardMedia
          component="img"
          height="200"
          image={item.image}
          ...
          <Box className={classes.helper}>
            {airports.activeAirport === item.id && loadingRoutes ? (
              ...
              <Box className={classes.action_hint}>
                ...
                >
                  Press to {airports.routes ? 'hide' : 'view'} routes
                </Typography>
              </Box>
            )}
          </Box>
